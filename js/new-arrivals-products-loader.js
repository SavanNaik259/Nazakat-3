...
const NewArrivalsProductsLoader = (function() {
    ...
    async function loadNewArrivalsProducts(forceRefresh = false) {
        ...
        try {
            ...
            console.log('Loading new arrivals products DIRECTLY from Firebase Storage CDN...');
            
            // DIRECT Firebase Storage CDN URL - NO PROXY LAYERS
            const directCDNUrl = `https://firebasestorage.googleapis.com/v0/b/auric-a0c92.firebasestorage.app/o/products%2Fnew-arrivals-products.json?alt=media`;
            
            console.log('🚀 DIRECT CDN ACCESS for New Arrivals:', directCDNUrl);
            ...
            if (response.status === 404) {
                console.log('New arrivals products file not found - will show empty state');
                return [];
            }
            ...
            try {
                ...
                if (newETag) {
                    cachedETag = newETag;
                    localStorage.setItem('newArrivalsProductsETag', newETag);
