...
async function sendCustomerOrderConfirmation(orderData) {
  try {
    ...
    const mailOptions = {
      from: `"Auric Jewelry" <${functions.config().email?.user || 'auricbysubha.web@gmail.com'}>`,
      to: customer.email,
...
async function sendOwnerOrderNotification(orderData) {
  try {
    ...
    const ownerEmail = functions.config().email?.owner || 'auricbysubha.web@gmail.com';
    ...
    // Define email options
    const mailOptions = {
      from: `"Auric Jewelry Orders" <${functions.config().email?.user || 'auricbysubha.web@gmail.com'}>`,
      to: ownerEmail,
